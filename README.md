# 평가용 Repositories
1차 평가 때 설명이 빈약했던 한이음 공모전, Tetris 레포지의 설명을 좀 더 자세히 덧붙였으며, 캡스톤디자인2020에 참여하며 만들었던 프로젝트도 새롭게 추가하였습니다. 딥러닝 모델을 구현한 레포지들은 pytorch문법도 잘 몰랐던 제가 직접 논문들을 읽고 from scratch로 하나하나씩 구현한 모델들입니다. 하나의 논문을 이해하기 위해선 많은 공부들이 필요했고 논문을 이해하는 거와 별개로 그 논문을 코드로 직접 구현하는 것 또한 굉장히 어려운 일이었습니다. 논문들은 이론적 설명만 적혀있을 뿐 구체적으로 어떤 방식으로 코드를 짜는지에 대한 설명은 없었기에 모델을 구현하면서 막힐 때마다 검색해가며 정말 많이 배우고 그만큼 또 실력이 늘었던 것 같습니다. 각 모델마다 코드가 복잡해서 코드별로 상세히 설명하기는 힘들었지만 각 논문에서 이루고자하는 목표나 모델들의 특징들이 잘 드러나게끔 설명하기위해 노력했습니다.

1. [summary.md](summary.md)
2. [CapstoneDesign2020](https://github.com/63days/CapstoneDesign20)
3. [MAML](https://github.com/63days/MAML)
4. [AugMix](https://github.com/63days/augmix)
5. [SimCLR](https://github.com/63days/SimCLR)
6. [FBS](https://github.com/63days/FBS-practice)
7. [SAT](https://github.com/63days/SAT)
8. [FGSM](https://github.com/63days/FGSM_practice)
9. [GCN & GAT](https://github.com/63days/GAT)
10. [RotNet-OOD](https://github.com/63days/RotNet-OOD)
11. [한이음 공모전](https://github.com/63days/hanium)
12. [Tetris](https://github.com/63days/tetris)
13. [알고리즘 문제 풀이](https://github.com/63days/algorithm)
